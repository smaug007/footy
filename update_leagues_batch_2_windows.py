#!/usr/bin/env python3
"""
BATCH 2: AMERICAS + ASIA LEAGUES DATA UPDATE (Windows Compatible)
================================================================
Updates completed matches and their statistics (goals + corners) for Americas and Asian leagues.
Run this on Wednesday after Batch 1 (European leagues) is complete.

League Coverage: 15 Americas + Asia leagues (including MLS, Liga MX, Brazilian Serie A, etc.)
Estimated API calls: ~375 calls (well within 7500 daily limit)
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(f'batch2_update_{datetime.now().strftime("%Y%m%d_%H%M%S")}.log'),
        logging.StreamHandler()
    ]
)
logger = logging.getLogger(__name__)

# Target leagues for cards updates (user specified)
CARDS_ENABLED_LEAGUES = {
    ...
    'Major League Soccer'   # USA (THIS IS IN BATCH 2!)
}

class AmericasAsiaLeaguesUpdater:
    """Updates completed match data for Americas and Asian leagues"""
    ...
    def __init__(self):
        ...
        logger.info("AMERICAS + ASIA LEAGUES UPDATER INITIALIZED")
        logger.info(f"Update Date: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}")
    
    def get_americas_asia_leagues(self) -> List:
        """Get all Americas and Asian leagues for batch processing"""
        all_leagues = self.league_manager.get_active_leagues()
        
        # Americas and Asian countries
        target_countries = {
            # Americas
            'United States', 'Mexico', 'Canada', 'Brazil', 'Argentina', 
            'Chile', 'Colombia', 'Uruguay', 'Peru', 'Ecuador', 'Paraguay',
            # Asia
            'China', 'Japan', 'South Korea'
        }
        
        americas_asia_leagues = [
            league for league in all_leagues 
            if league.country in target_countries
        ]
        
        logger.info(f"Found {len(americas_asia_leagues)} Americas + Asia leagues to update")
        return sorted(americas_asia_leagues, key=lambda x: x.priority_order)
    ...
    def update_league_completed_matches(self, league) -> Dict[str, int]:
        ...
        try:
            ...
            # Step 2: Update match statistics
            for i, match_id in enumerate(all_match_ids, 1):
                try:
                    ...
                    if fixture_details:
                        ...
                        # Try to get corners and cards from raw_data statistics
                        ...
                        if 'statistics' in raw_data:
                            ...
                            for team_stats in statistics:
                                ...
                                for stat in team_stats.get('statistics', []):
                                    # Extract corners (existing logic)
                                    ...
                                    # Extract cards (NEW - only for target leagues)
                                    if league.name in CARDS_ENABLED_LEAGUES:
                                        if stat.get('type') == 'Yellow Cards':
                                            cards_value = stat.get('value') or 0
                                            try:
                                                cards_count = int(cards_value)
                                            except (ValueError, TypeError):
                                                cards_count = 0
                                            
                                            if team_type == 'home':
                                                yellow_home = cards_count
                                            else:
                                                yellow_away = cards_count
                                            ...
                                            if team_type == 'home':
                                                red_home = cards_count
                                            else:
                                                red_away = cards_count
                        
                        # Update database
                        with self.db_manager.get_connection() as conn:
                            # Conditionally update with or without cards
                            if league.name in CARDS_ENABLED_LEAGUES:
                                ...
                                if cursor.rowcount > 0:
                                    ...
                                    logger.info(f"      SUCCESS: {goals_home}-{goals_away} (goals), {corners_home}-{corners_away} (corners), Y:{yellow_home}-{yellow_away} R:{red_home}-{red_away} (cards)")
                            else:
            ...
            if league.name in CARDS_ENABLED_LEAGUES:
                logger.info(f"   COMPLETE: {league.name} - {results['statuses_updated']} statuses, {results['matches_processed']} matches, {results['goals_updated']} goals, {results['corners_updated']} corners, {results['cards_updated']} cards")
            else:
    ...
    def run_batch_update(self):
        """Run the complete Americas + Asia leagues batch update"""
        
        logger.info("STARTING BATCH 2: AMERICAS + ASIA LEAGUES UPDATE")
        logger.info("=" * 80)
        
        start_time = datetime.now()
        americas_asia_leagues = self.get_americas_asia_leagues()
        ...
        try:
            for i, league in enumerate(americas_asia_leagues, 1):
                self.leagues_processed += 1
                
                logger.info(f"\nLEAGUE {i}/{len(americas_asia_leagues)}: {league.name}")
                logger.info("-" * 60)
                ...
                # Progress update
                logger.info(f"BATCH PROGRESS: {i}/{len(americas_asia_leagues)} leagues, {self.api_calls_used} API calls used")
        ...
        logger.info(f"Leagues:  {total_results['leagues_processed']}/{len(americas_asia_leagues)}")
...
def main():
    ...
    print("AMERICAS + ASIA LEAGUES DATA UPDATER")
    print("=" * 50)
    print("This will update completed match statistics for Americas and Asian leagues.")
    print("Leagues: MLS, Liga MX, Brazilian Serie A, J1 League, K League 1, etc.")
    print("Estimated API calls: ~375 calls")
    ...
    try:
        updater = AmericasAsiaLeaguesUpdater()
